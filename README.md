...
**Predicting arrival delays before departure using pre-flight information — and quantifying where airlines lose the most time.**

Dataset: US DOT Bureau of Transportation Statistics — On-Time Performance, 2024 (582,425 domestic flights, 10 carriers, 5,880 routes)
...
1. **Where** does the delay risk concentrate (route, carrier, day, time, season)?
2. **Can we predict**, using only information known *before* departure, whether a flight will arrive 15+ minutes late?
...
**Target:** `ArrDel15` — will the flight arrive 15+ minutes late? (binary classification)
**Features used (deliberately limited to pre-departure information only, to avoid data leakage):** carrier, origin, destination, scheduled departure hour, distance, month, day of week, quarter.
